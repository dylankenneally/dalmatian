...
```bash
dalmatian$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
dalmatian$ sam local start-api
dalmatian$ curl http://localhost:3000/
```
...
```bash
dalmatian$ sam logs -n HelloWorldFunction --stack-name dalmatian --tail
```
...
```bash
sam delete --stack-name dalmatian
```
